...
- Este é um projeto desenvolvido para praticar `JavaScript` e `Jest`;
...
Em uma simulação, sabendo das minhas habilidades com ES6, Higher Order Functions e testes, a prefeitura da cidade me deu uma importante missão: organizar as informações do zoológico! :elephant:

Sou responsável por desenvolver funções que buscam informações sobre os animais do zoológico como: espécie e local de origem. Além disso, tive que também buscar dados sobre as pessoas que colaboram com a manutenção e cuidado do zoológico. :farmer:
...
1. Clone o repo:
    ...
    ```bash
    git clone git@github.com:Ludson96/project-zoo-functions.git
    ```
...
1. Todas as funções se encontram na pasta `src` para executar os testes basta usar o comando `npm test` para executar todos os testes ou `npm test <nome do teste>` para executar um teste especifico. A pasta `data` contem o arquivo `zoo_data` fornecido pela **Trybe**.

1. Abaixo segue os requisitos proposto pela **Trybe**
